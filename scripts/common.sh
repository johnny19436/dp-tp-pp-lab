#!/usr/bin/env bash
# Shared helpers for DP/TP/PP lab under /root/working/dp-tp-pp-lab
set -euo pipefail
...
# Frozen model config (plan §5). Change only if 1GPU OOMs, then freeze for all.
...
# Build arg array in-place (avoid mapfile / word-splitting issues with new Megatron CLI)
...
run_megatron() {
  ...
  echo "[$(date -u +%FT%TZ)] START ${RUN_ID} nproc=${nproc}" | tee "${logfile}"
  ...
  echo "[$(date -u +%FT%TZ)] END ${RUN_ID}" | tee -a "${logfile}"
}
